...
USERLIST_CFG=${SCRIPTS_PATH}/../config/cred_list_user_pwd.cfg
...
grep -v '^#' ${USERLIST_CFG} | while IFS=: read -r f1 f2 f3 f4 f5
    do
			...
			java -jar ${SCRIPTS_PATH}/jenkins-cli.jar -s ${JENKINS_URL} groovy ${SCRIPTS_PATH}/AddUserPwdCred.groovy "$f2" "$f3" "$f4" "$f5" --username ${JENKINS_USER} --password ${JENKINS_PWD} | tee -a $FILE_LOG
			sleep 5
			java -jar ${SCRIPTS_PATH}/jenkins-cli.jar -s ${JENKINS_URL} list-credentials ${JENKINS_STORE} --username ${JENKINS_USER} --password ${JENKINS_PWD} | grep $f4 | tee -a $FILE_LOG
			sleep 5
			echo "																																									" | tee -a $FILE_LOG
		done
