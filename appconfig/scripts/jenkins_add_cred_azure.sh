#########################################################################################
#  Description:	 Script to Add AZURE Credentials in Jenkins- jenkins_add_cred_azure.sh
#  Author: Bhawna Goyal
#  Date: 19/03/2017
#  Action History
...
AZURE_Conf=${SCRIPTS_PATH}/../config/AZURE_Credential.xml
...
Description=$(grep description ${AZURE_Conf}|cut -d'>' -f2|cut -d'<' -f1)

echo "Adding AZURE Credentials with Description- ${Description}...." | tee -a $FILE_LOG
cat ${AZURE_Conf} | java -jar ${SCRIPTS_PATH}/jenkins-cli.jar -s ${JENKINS_URL} create-credentials-by-xml ${JENKINS_STORE} ${JENKINS_DOMAIN} --username ${JENKINS_USER} --password ${JENKINS_PWD} | tee -a $FILE_LOG
sleep 5
