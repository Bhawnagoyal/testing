#########################################################################################
#  Description:	 Script to add Jenkins Credentials SSH User with Private Key- jenkins_add_cred_ssh_user_pvtkey.sh
#  Author: Bhawna Goyal
#  Date: 19/03/2017
#  Action History
...
exit_error_parameter() {
	...
	echo "Usage: $(basename $0) <id> <description> <username>"
	echo "	-h: Help"
	exit 1
}
...
echo " - Credential ID    	-> $1" | tee -a  $FILE_LOG
echo " - Credential description    -> $2" | tee -a  $FILE_LOG
echo " - Credential username    	-> $3" | tee -a  $FILE_LOG
echo "-----------------------------------------------------------------------------" | tee -a  $FILE_LOG
echo "                                                                             " | tee -a $FILE_LOG
...
echo "Adding SSH User with Private Key....." | tee -a $FILE_LOG
java -jar ${SCRIPTS_PATH}/jenkins-cli.jar -s ${JENKINS_URL} groovy ${SCRIPTS_PATH}/AddSSHUserPrivateKey.groovy "$1" "$2" "$3" --username ${JENKINS_USER} --password ${JENKINS_PWD} | tee -a $FILE_LOG
sleep 5
java -jar ${SCRIPTS_PATH}/jenkins-cli.jar -s ${JENKINS_URL} list-credentials ${JENKINS_STORE} --username ${JENKINS_USER} --password ${JENKINS_PWD} | grep $3 | tee -a $FILE_LOG
sleep 5
echo "																																									" | tee -a $FILE_LOG
