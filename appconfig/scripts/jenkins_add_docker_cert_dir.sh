#########################################################################################
#  Description:	 Script to Add Docker Certificate directory Credentials in Jenkins-
#  Author: Bhawna Goyal
#  Date: 19/03/2017
#  Action History
...
exit_error_parameter() {
	echo "Script to List Jenkins Credentials - "
...
DOCKER_Conf=${SCRIPTS_PATH}/../config/DOCKER_Cert_Dir.xml
...
Description=$(grep description ${DOCKER_Conf}|cut -d'>' -f2|cut -d'<' -f1)

echo "Adding Docker Certificate Directory Credentials with Description- ${Description}...." | tee -a $FILE_LOG
cat ${DOCKER_Conf} | java -jar ${SCRIPTS_PATH}/jenkins-cli.jar -s ${JENKINS_URL} create-credentials-by-xml ${JENKINS_STORE} ${JENKINS_DOMAIN} --username ${JENKINS_USER} --password ${JENKINS_PWD} | tee -a $FILE_LOG
sleep 5
