#########################################################################################
#  Description:	 Script to Add AWS Credentials in Jenkins- jenkins_add_cred_aws.sh
#  Author: Bhawna Goyal
#  Date: 19/03/2017
#  Action History
...
AWS_Conf=${SCRIPTS_PATH}/../config/AWS_Credential.xml
...
Description=$(grep description ${AWS_Conf}|cut -d'>' -f2|cut -d'<' -f1)

echo "Adding AWS Credentials with Description- ${Description}...." | tee -a $FILE_LOG
cat ${AWS_Conf} | java -jar ${SCRIPTS_PATH}/jenkins-cli.jar -s ${JENKINS_URL} create-credentials-by-xml ${JENKINS_STORE} ${JENKINS_DOMAIN} --username ${JENKINS_USER} --password ${JENKINS_PWD} | tee -a $FILE_LOG
sleep 5
