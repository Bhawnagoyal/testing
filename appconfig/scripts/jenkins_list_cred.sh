#########################################################################################
#  Description:	 Script to List Jenkins Credentials - jenkins_list_cred.sh
#  Author: Bhawna Goyal
#  Date: 19/03/2017
#  Action History
...
java -jar ${SCRIPTS_PATH}/jenkins-cli.jar -s ${JENKINS_URL} list-credentials ${JENKINS_STORE} --username ${JENKINS_USER} --password ${JENKINS_PWD} | tee -a $FILE_LOG
if [ $? -ne 0 ]
then
     exit_error_validation
fi
