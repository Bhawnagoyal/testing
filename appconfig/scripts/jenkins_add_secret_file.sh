#########################################################################################
#  Description:	 Script to Add Secret File Credentials in Jenkins
#  Author: Bhawna Goyal
#  Date: 19/03/2017
#  Action History
...
File_Conf=${SCRIPTS_PATH}/../config/Cred_Secret_File.xml
...
Description=$(grep description ${File_Conf}|cut -d'>' -f2|cut -d'<' -f1)

echo "Adding Secret File Credentials with Description- ${Description}...." | tee -a $FILE_LOG
cat ${File_Conf} | java -jar ${SCRIPTS_PATH}/jenkins-cli.jar -s ${JENKINS_URL} create-credentials-by-xml ${JENKINS_STORE} ${JENKINS_DOMAIN} --username ${JENKINS_USER} --password ${JENKINS_PWD} | tee -a $FILE_LOG
sleep 5
