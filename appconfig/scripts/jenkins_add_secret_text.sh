#########################################################################################
#  Description:	 Script to Add Secret Text Credentials in Jenkins
#  Author: Bhawna Goyal
#  Date: 19/03/2017
#  Action History
...
Text_Conf=${SCRIPTS_PATH}/../config/Cred_Secret_Text.xml
...
Description=$(grep description ${Text_Conf}|cut -d'>' -f2|cut -d'<' -f1)

echo "Adding Secret Text Credentials with Description- ${Description}...." | tee -a $FILE_LOG
cat ${Text_Conf} | java -jar ${SCRIPTS_PATH}/jenkins-cli.jar -s ${JENKINS_URL} create-credentials-by-xml ${JENKINS_STORE} ${JENKINS_DOMAIN} --username ${JENKINS_USER} --password ${JENKINS_PWD} | tee -a $FILE_LOG
sleep 5
