#########################################################################################
#  Description:	 Script to add NodeJS Installation-
#  Author: Bhawna Goyal
#  Date: 19/03/2017
#  Action History
...
exit_error_parameter() {
	echo "Script to add NodeJS Installation - "
...
NODEJS_CFG=${SCRIPTS_PATH}/../config/nodejs_conf.cfg
...
Nodejs_Name=$(cat ${NODEJS_CFG} | grep nodejs_name | cut -d= -f2)
Install_Dir=$(cat ${NODEJS_CFG} | grep install_dir | cut -d= -f2)
Nodejs_Version=$(cat ${NODEJS_CFG} | grep nodejs_version | cut -d= -f2)
Npm_Package=$(cat ${NODEJS_CFG} | grep npm_package | cut -d= -f2)


echo "Adding NodeJS Installation....." | tee -a $FILE_LOG
java -jar ${SCRIPTS_PATH}/jenkins-cli.jar -s ${JENKINS_URL} groovy ${SCRIPTS_PATH}/add_nodejs_auto_installer.groovy "$Nodejs_Name" "$Install_Dir" "$Nodejs_Version" "$Npm_Package" --username ${JENKINS_USER} --password ${JENKINS_PWD} | tee -a $FILE_LOG
echo "																																									" | tee -a $FILE_LOG
