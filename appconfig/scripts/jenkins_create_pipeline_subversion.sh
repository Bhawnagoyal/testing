#########################################################################################
#  Description:	 Script to create pipeline using subversion in Jenkins- jenkins_create_pipeline_subversion.sh
#  Author: Bhawna Goyal
#  Date: 19/03/2017
#  Action History
...
exit_error_parameter() {
	echo "Script to Create Jenkins pipeline - "
...
Pipeline_Conf=${SCRIPTS_PATH}/../config/pipeline-subversion.xml
Pipeline_Name=pipeline-subversion
...
Description=$(grep description ${Pipeline_Conf}|cut -d'>' -f2|cut -d'<' -f1)

echo "Creating Jenkins Pipeline with Description- ${Description}...." | tee -a $FILE_LOG
java -jar ${SCRIPTS_PATH}/jenkins-cli.jar -s ${JENKINS_URL} create-job ${Pipeline_Name} < ${Pipeline_Conf} --username ${JENKINS_USER} --password ${JENKINS_PWD} | tee -a $FILE_LOG
sleep 5
echo "                                                        " | tee -a $FILE_LOG
echo "Pipeline - ${Pipeline_Name} created succesfully" | tee -a $FILE_LOG
echo "                                                        " | tee -a $FILE_LOG
echo "List of Available Pipeline and Jobs in Jenkins as below:" | tee -a $FILE_LOG
java -jar ${SCRIPTS_PATH}/jenkins-cli.jar -s ${JENKINS_URL} list-jobs --username ${JENKINS_USER} --password ${JENKINS_PWD} | tee -a $FILE_LOG
sleep 5
echo "                                                                             " | tee -a $FILE_LOG
exit 0
